...
if __name__ == '__main__':
    ...
    for o, a in opt_list:
        ...
        if o in ('-p', '--p'):
            ...
            if verbose: print
            'preserve initial charges on ', preserve_charge_types
        if o in ('-U', '--U'):
            cleanup = a
            if verbose: print
            'set cleanup to merge ', a
        if o in ('-B', '--B'):
            allowed_bonds = a
            if verbose: print
            'allow ', a, 'bonds set to rotate'
        if o in ('-R', '--R'):
            root = a
            if verbose: print
            'set root to ', root
        if o in ('-F', '--F'):
            ...
            if verbose: print
            'set check_for_fragments to True'
        if o in ('-M', '--M'):
            mode = a
            if verbose: print
            'set mode to ', a
        if o in ('-I', '--I'):
            ...
            if verbose: print
            'set bonds_to_inactivate to ', a
        if o in ('-Z', '--Z'):
            inactivate_all_torsions = True
            if verbose: print
            'set inactivate_all_torsions to ', inactivate_all_torsions
        if o in ('-g', '--g'):
            attach_nonbonded_fragments = True
            if verbose: print
            'set attach_nonbonded_fragments to ', attach_nonbonded_fragments
        if o in ('-s', '--s'):
            attach_singletons = True
            if verbose: print
            'set attach_singletons to ', attach_singletons
        if o in ('-w', '--w'):
            assign_unique_names = True
            if verbose: print
            'set assign_unique_names to ', assign_unique_names, ' newname is original name plus its index(1-based'
        if o in ('-h', '--'):
            usage()
            sys.exit()
    ...
    if attach_singletons:
        attach_nonbonded_fragments = True
        if verbose: print
        "using attach_singletons so attach_nonbonded_fragments also"

    mols = Read(ligand_filename)
    if verbose: print
    'read ', ligand_filename
    mol = mols[0]
    if len(mols) > 1:
        if verbose:
            print
    ...
    if verbose:
        print
        "setting up LPO with mode=", mode,
        print
        "and outputfilename= ", outputfilename
        print
        "and check_for_fragments=", check_for_fragments
        print
        "and bonds_to_inactivate=", bonds_to_inactivate
    ...
    if charges_to_add is not None:
        ...
        for atom, chargeList in preserved.items():
            ...
            if verbose: print
            "set charge on ", atom.full_name(), " to ", atom.charge
    if verbose: print
    "returning ", mol.returnCode
    bad_list = []
